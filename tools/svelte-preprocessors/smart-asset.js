...
async function collectAssets(content) {
  ...
  await svelte.walk(ast, {
    enter: (node) => {
      if (!node.attributes || node.attributes.length === 0 || !['Element', 'Fragment', 'InlineComponent'].includes(node.type)) {
        return;
      }
      const candidates = node.attributes.filter(
        (attribute) =>
          ['src', 'href', 'xlink:href'].includes(attribute.name) && attribute.value.length === 1 && attribute.value[0].type === 'Text',
      );
...
async function process(assets, options) {
  const processes = assets.map((asset) =>
    cssProcessor
      .process(
      ...
      .then((result) => (asset.updatedPath = result.css.slice(25, -3))),
  ); // extract new path from css after postcss-smart-asset
